...
class SSTVDecoder(object):
    ...
    def decode(self, skip=0.0, forced_mode=None, progress_callback=None):
        """Attempts to decode the audio data as an SSTV signal.

        Args:
            skip: Seconds to skip at start of audio.
            forced_mode: Optional mode class (e.g. spec.R36) to use instead of VIS detection.
            progress_callback: Optional callable(lines_decoded, total_lines, partial_image_data, mode)
                              called after each decoded line for waterfall display.
        ...
        image_data = self._decode_image_data(vis_end, progress_callback=progress_callback)
    ...
    def _decode_image_data(self, image_start, progress_callback=None):
        ...
        for line in range(height):
            ...
            if progress_callback:
                part = [[[image_data[y][c][x] for x in range(width)] for c in range(channels)] for y in range(line + 1)]
                progress_callback(line + 1, height, part, self.mode)
...
def draw_partial_image(mode, image_data_partial, num_lines):
    """Build PIL Image from first num_lines of image_data (for waterfall display).
    image_data_partial: list of length num_lines, each [channels][width].
    """
    if num_lines <= 0 or not image_data_partial:
        return Image.new("RGB", (mode.LINE_WIDTH, 1), (0, 0, 0))
    width = mode.LINE_WIDTH
    height = num_lines
    channels = mode.CHAN_COUNT
    data = image_data_partial
    n = len(data)

    if mode.COLOR == spec.COL_FMT.YUV:
        col_mode = "YCbCr"
    else:
        col_mode = "RGB"
    image = Image.new(col_mode, (width, height))
    ...
    def clamp_line(yy):
        return min(max(yy, 0), n - 1)

    for y in range(height):
        odd_line = y % 2
        for x in range(width):
            if channels == 2:
                if mode.HAS_ALT_SCAN and mode.COLOR == spec.COL_FMT.YUV:
                    y0 = clamp_line(y)
                    y1 = clamp_line(y - (odd_line - 1))
                    y2 = clamp_line(y - odd_line)
                    pixel = (data[y0][0][x], data[y1][1][x], data[y2][1][x])
                else:
                    pixel = (data[y][0][x], data[y][1][x], data[y][1][x])
            elif channels == 3:
                if mode.COLOR == spec.COL_FMT.GBR:
                    pixel = (data[y][2][x], data[y][0][x], data[y][1][x])
                elif mode.COLOR == spec.COL_FMT.YUV:
                    pixel = (data[y][0][x], data[y][2][x], data[y][1][x])
                elif mode.COLOR == spec.COL_FMT.RGB:
                    pixel = (data[y][0][x], data[y][1][x], data[y][2][x])
                else:
                    pixel = (data[y][0][x], data[y][1][x], data[y][2][x])
            pixel_data[x, y] = pixel
